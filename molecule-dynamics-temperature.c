/*
 *  molecule-dynamics-temperature.c - program to simulate argon like gas
 *                                    behavior in the thermodynamic system
 *                                    with constant temperature.
 *
 *  This program was written for 3-d course students of Statistical Physics
 *  Department of Saint-Petersburg state university of Russia, so all texts
 *  and comments in the program are in russian (utf-8 character set is used).
 *
 *  The program written for Unix like operation system, it will not run and
 *  even compile under Windows like operation system without modification.
 *  You'll need to install gnuplot(>= 4.2) and libgsl (GNU Scientific Library)
 *  to compile and run this application properly.
 *
 *  Copyright (C) 2009, 20010 Mikhail Kshevetskiy <mikhail.kshevetskiy@gmail.com>
 *
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 *
...
void initialize(vector_array *r, vector_array *v, int count, double Vmax){
    ...
    dl = pow((Lx * Ly * Lz) / count, 1.0 / 3.0);
    nx = ceil(Lx / dl);
    ny = ceil(Ly / dl);
    nz = ceil(Lz / dl);

    /* по идее этот код не нужен, но я параноик */
    while (nx * ny * nz < count){
	if ((Lx >= Ly) && (Lx >= Lz)) { nx++; continue; };
	if ((Ly >= Lx) && (Ly >= Lz)) { ny++; continue; };
	if ((Lz >= Lx) && (Lz >= Ly)) { nz++; continue; };
    }
