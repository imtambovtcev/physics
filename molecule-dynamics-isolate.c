...
double calc_accel_and_potential(vector_array *r, vector_array *a, int count){
	...
	for(j = i + 1; j < count; j++){
	    ...
	    /* учтем периодические условия и будем учитывать     */
	    /* взаимодействие только c ближайшей частицей.       */
	    /* Внимание: Lx, Ly, Lz должны быть больше 3 * sigma */
	    if (dx >   Lx / 2) dx -= Lx;
	    if (dx < - Lx / 2) dx += Lx;
	    if (dy >   Ly / 2) dy -= Ly;
	    if (dy < - Ly / 2) dy += Ly;
...
void time_step(vector_array *r, vector_array *v, vector_array *a, int count,
		double dt, double *kinetic_energy, double *potential_energy){
    int		i;
	...
	if (r->x[i] >= Lx) r->x[i] -= Lx;
	if (r->y[i] >= Ly) r->y[i] -= Ly;
	if (r->z[i] >= Lz) r->z[i] -= Lz;
    }
...
void save_snapshot(vector_array *r, vector_array *v, int count,
    ...
    save_data_file(name, r, v, count, kinetic_energy, potential_energy, t);
}
...
int action_request(vector_array *r, vector_array *v, int count,
	...
	switch(ch){
		...
		space_distribution(r, v, count, kinetic_energy, potential_energy, t);
		continue;

	    case '3':	/* Сохраним данные в файл */
		save_snapshot(r, v, count, kinetic_energy, potential_energy, t);
		continue;
...
int main(int argc, char *argv[]){
    ...
    printf("Моделирование микроканонического ансамбля\n\n");
	...
	switch(c){
		...
		if (load_data_file(optarg, &count, &r, &v, &t) != 1){
		    exit(EXIT_FAILURE);
		}
		load_flag = 1;
		break;
	...
	time_step(&r, &v, &a, count,
	    dt, &kinetic_energy, &potential_energy);
	if (current_time != last_time){
	    last_time = current_time;
	    printf("\rt=%lf,  Ek=%lf,  Ep=%lf,  E=%lf       ",
